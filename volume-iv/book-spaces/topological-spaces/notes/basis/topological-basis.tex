...
O=\bigcup_{B\in\mathcal{A}}B
\]
means that \(\mathcal{B}\) is a basis for the topology \(\tau\).
\end{remark*}
...
x\in B\subseteq O
\]
means that \(\mathcal{B}\) is a basis exactly when it supplies local basic
open neighborhoods inside every open set.
\end{remark*}
