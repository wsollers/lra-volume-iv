...
1. () `lem:metric-key-local-ball` — **Key Local Ball Lemma**
   > **Statement.**
...
3. () `thm:metric-ball-containment-by-center-distance` — **Ball Containment by Center Distance**
   > **Statement.**
...
4. () `thm:metric-intersecting-balls-center-distance-bound` — **Intersecting Balls Imply Center-Distance Bound**
   > **Statement.**
...
5. () `thm:metric-closed-balls-contain-open-balls` — **Closed Balls Contain Open Balls**
   > **Statement.**
...
6. () `thm:metric-distance-to-union` — **Distance to a Union**
   > **Statement.**
...
7. () `thm:metric-distance-to-intersection-bound` — **Distance to an Intersection Bound**
   > **Statement.**
...
9. () `thm:metric-bounded-center-independence` — **Center Independence**
   > **Statement.**
...
10. () `thm:metric-subsets-bounded-sets-bounded` — **Subsets of Bounded Sets Are Bounded**
   > **Statement.**
   > Let \((X,d)\) be a metric space. If \(A\subseteq B\subseteq X\) and \(B\) is
   > bounded, then \(A\) is bounded.

11. () `thm:metric-finite-sets-bounded` — **Finite Sets Are Bounded**
   > **Statement.**
   > Every finite subset of a metric space is bounded.

12. () `thm:metric-finite-unions-bounded-sets-bounded` — **Finite Unions of Bounded Sets Are Bounded**
   > **Statement.**
...
13. () `thm:metric-boundedness-via-diameter` — **Boundedness via Diameter**
   > **Statement.**
...
14. () `thm:metric-closure-bounded-set-bounded` — **Closure of a Bounded Set Is Bounded**
   > **Statement.**
   > Let \((X,d)\) be a metric space. If \(A\subseteq X\) is bounded, then
   > \(\operatorname{cl}(A)\) is bounded.

15. () `thm:metric-compact-sets-bounded` — **Compact Sets Are Bounded**
   > **Statement.**
   > Every compact subset of a metric space is bounded.

16. () `thm:metric-totally-bounded-sets-bounded` — **Totally Bounded Sets Are Bounded**
   > **Statement.**
   > Every totally bounded subset of a metric space is bounded.

17. () `lem:metrics-are-pseudo-metrics` — **Metrics are Pseudo-Metrics**
   > **Statement.**
...
18. () `thm:metric-closed-sets-contain-limit-points` — **Closed Sets Contain Their Limit Points**
   > **Statement.**
...
21. () `thm:cauchy-schwarz-inequality` — **Cauchy--Schwarz Inequality**
   > **Statement.**
...
22. () `cor:minkowski-inequality-rn` — **Minkowski's Inequality**
   > **Statement.**
   > For every \(x,y\in\mathbb{R}^n\),
   > \[
   >  |x+y|\leq |x|+|y|.
...
23. () `cor:euclidean-metric` — **Euclidean Metric**
   > **Statement.**
...
24. () `thm:metric-euclidean-bolzano-weierstrass` — **Bolzano--Weierstrass in \(\mathbb{R}^m\)**
   > **Statement.**
   > Every bounded sequence in \(\mathbb{R}^m\) contains a subsequence that
   > converges in \(\mathbb{R}^m\).

25. () `thm:metric-bolzano-weierstrass-bounded-infinite-subsets` — **Bolzano--Weierstrass for Bounded Infinite Subsets**
   > **Statement.**
   > Every bounded infinite subset of \(\mathbb{R}^m\) has a limit point in
   > \(\mathbb{R}^m\).

26. () `prop:metric-derived-set-closed` — **The Derived Set is Closed**
   > **Statement.**
   > The set of limit points of any set is closed.

27. () `thm:norm-induces-metric` — **A Norm Induces a Metric**
   > **Statement.**
...
28. () `prop:discrete-metric-space` — **The Discrete Distance is a Metric**
   > **Statement.**
   > For every nonempty set \(X\), the discrete distance
   > \(d_{\mathrm{disc}}\) is a metric on \(X\).

29. () `prop:real-line-euclidean-distance-metric` — **The Real-Line Euclidean Distance is a Metric**
   > **Statement.**
   > The function \(d_{\mathbb{R}}(x,y)=|x-y|\) is a metric on \(\mathbb{R}\).

30. () `prop:euclidean-distance-rn-metric` — **The Euclidean Distance on \(\mathbb{R}^n\) is a Metric**
   > **Statement.**
   > The function \(d_2\) is a metric on \(\mathbb{R}^n\).

31. () `prop:taxicab-metric-rn` — **Taxicab Metric**
   > **Statement.**
   > The function \(d_1\) is a metric on \(\mathbb{R}^n\).

32. () `thm:lp-metric-rn` — **\(d_p\) is a Metric**
   > **Statement.**
   > For every \(1\leq p<\infty\), the function \(d_p\) is a metric on
   > \(\mathbb{R}^n\).

33. () `prop:complex-plane-metric-space` — **The Complex Plane is a Metric Space**
   > **Statement.**
   > The pair \((\mathbb{C},d_{\mathbb{C}})\) is a metric space.

34. () `prop:bounded-real-sequence-space-sup-metric` — **The Supremum Distance on Bounded Sequences is a Metric**
   > **Statement.**
   > The function \(d_\infty\) is a metric on \(\ell^\infty(\mathbb{R})\).

35. () `thm:metric-point-functions-identify-points` — **Point Functions Identify Points**
   > **Statement.**
...
36. () `thm:metric-point-function-inequalities` — **Point Function Inequalities**
   > **Statement.**
...
37. () `thm:metric-pointlike-zero-point-function` — **Pointlike Functions with a Zero Are Point Functions**
   > **Statement.**
...
38. () `thm:metric-open-balls-are-open` — **Open Balls are Open**
   > **Statement.**
   > Let \((X,d)\) be a metric space. For every \(x\in X\) and every \(r>0\),
   > the open ball \(B_d(x;r)\) is open in \((X,d)\).

39. () `thm:metric-open-set-closure-properties` — **Metric Open Set Closure Properties**
   > **Statement.**
...
40. () `thm:metric-complement-closed-ball-open` — **Complement of a Closed Ball Is Open**
   > **Statement.**
...
41. () `thm:metric-closed-balls-are-closed` — **Closed Balls Are Closed**
   > **Statement.**
   > Let \((X,d)\) be a metric space. For every \(x\in X\) and every \(r\geq 0\),
   > the closed ball
   > \[
...
42. () `thm:metric-closure-open-ball-contained-closed-ball` — **Closure of an Open Ball Is Contained in the Closed Ball**
   > **Statement.**
   > Let \((X,d)\) be a metric space. For every \(x\in X\) and every \(r>0\),
   > \[
...
43. () `thm:metric-interior-largest-open-subset` — **Interior is the Largest Open Subset**
   > **Statement.**
...
44. () `thm:metric-neighborhood-criterion-sequential-convergence` — **Metric Neighborhood Criterion for Sequential Convergence**
   > **Statement.**
...
45. () `thm:metric-convergent-sequences-unique-limits` — **Metric Convergent Sequences Have Unique Limits**
   > **Statement.**
   > In metric spaces, convergent sequences have unique limits.

46. () `thm:metric-cauchy-sequence-convergent-subsequence` — **Metric Cauchy Sequence with Convergent Subsequence**
   > **Statement.**
...
47. () `thm:metric-euclidean-coordinatewise-sequences` — **Coordinatewise Sequences in \(\mathbb{R}^m\)**
   > **Statement.**
...
48. () `thm:metric-euclidean-cauchy-sequences-converge` — **Euclidean Cauchy Sequences Converge**
   > **Statement.**
   > Every Cauchy sequence in \(\mathbb{R}^m\) is convergent in \(\mathbb{R}^m\).

49. () `prop:topological-basis-pointwise-characterization` — **Pointwise Characterization of a Basis**
   > **Statement.**
...
50. () `thm:closed-set-characterization-topologies` — **Closed-Set Characterization of Topologies**
   > **Statement.**
...
51. () `thm:sigma-intersection` — **Intersection of $\sigma$-algebras is a $\sigma$-algebra**
   > **Statement.**
...
52. () `thm:ordered-field-translation-invariant` — **Order Is Translation Invariant**
   > **Statement.**
   > In an ordered field, if \(a<b\), then for every \(c\), \(a+c<b+c\).

53. () `thm:ordered-field-positive-multiplication-preserves-order` — **Order Is Preserved by Positive Multiplication**
   > **Statement.**
   > In an ordered field, if \(a<b\) and \(c>0\), then \(ac<bc\).

54. () `thm:ordered-field-negative-multiplication-reverses-order` — **Order Is Reversed by Negative Multiplication**
   > **Statement.**
   > In an ordered field, if \(a<b\) and \(c<0\), then \(bc<ac\).

55. () `thm:ordered-field-positive-reciprocals` — **Positive Elements Have Positive Reciprocals**
   > **Statement.**
   > In an ordered field, if \(a>0\), then \(a^{-1}>0\).

56. () `thm:ordered-field-negative-reciprocals` — **Negative Elements Have Negative Reciprocals**
   > **Statement.**
   > In an ordered field, if \(a<0\), then \(a^{-1}<0\).

57. () `thm:ordered-field-squares-nonnegative` — **Squares Are Nonnegative**
   > **Statement.**
   > In an ordered field, for every \(x\), \(x^2\ge 0\).

58. () `thm:ordered-field-one-positive` — **One Is Positive**
   > **Statement.**
   > In an ordered field, \(1>0\).

59. () `thm:ordered-field-no-square-root-negative-one` — **No Ordered Field Has Square Root of Negative One**
   > **Statement.**
   > In an ordered field, there is no \(x\) such that \(x^2=-1\).

60. () `lem:ordered-field-product-sign-laws` — **Product Sign Laws in an Ordered Field**
   > **Statement.**
   > In an ordered field, the usual product sign laws hold.

61. () `lem:ordered-field-division-order-laws` — **Division Order Laws in an Ordered Field**
   > **Statement.**
